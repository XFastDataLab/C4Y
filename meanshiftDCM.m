...
%集中式聚类
%参数带宽
bandwidth1=0.15;     %总体的bandwidth   bandwidth=85  7.mat   0.15  synthesis_3.txt
bandwidth2=0.178;    %样本的bandwidth              85          0.178          103.5  
k=5;
%figure(),gscatter(dataall(:,1), dataall(:,2));
tic
%集中式聚类
% clustCent：聚类中心 D*K, cluster_idx：聚类结果 类标签, 1*N
[clustCent,cluster_idx,clustMembsCell] = MeanShiftCluster(dataall',bandwidth1);
toc
%作图
...
for i = 1:n
    ...
    [clustCent,cluster_idx{1,i},clustMembsCell] = MeanShiftCluster(ori_sample',bandwidth2);
    %toc
    %[cluster_idx{1, i} ,~]= kmeans(dione, k);
    %打印
